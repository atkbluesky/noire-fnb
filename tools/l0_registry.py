...
SOURCES = [
    ...
    dict(
        ...
        produces=["channel", "daypart", "identify", "fact_promo_day", "nature", "recon",
                  "daily_party", "heat", "zone", "staff", "payment", "dwell", "repeat"],
    ...
    dict(
        ...
        produces=["pre_eval", "pre_eval_scheme", "pre_eval_fin", "pre_eval_base", "pre_eval_input", "pre_eval_fix", "pre_plan_lock"],
    ...
    dict(
        id="S25_mkt_report", group="03_MARKETING", dir="03_MARKETING/08_Bao_Cao_MKT_Thang",
        name="Báo cáo marketing tháng (Promotion-AGG)", cadence="monthly", since="2026-08", required=False,
        pattern="*Promotion AGG*.xlsx", month_regex=r"T(\d{1,2})-(\d{4})",
        example="NOIRE_Bao_Cao_Promotion AGG - MKT_T8-2026.xlsx",
        how="Báo cáo marketing tháng ĐẦY ĐỦ của team (Scorecard · Aggregator · Partnership · Budget…), tên có `T<tháng>-<năm>`. Hệ thống chỉ đọc sheet Budget (chi phí ngoài media: KOL · POSM · sản xuất · sự kiện). Số aggregator nhập ở 05_DOI_TAC/03_Aggregator.",
        produces=["budget_nonmedia"], via="tools/build_month.py", modules=["M4"],
    ),
    dict(
        ...
        produces=["dim_campaign", "campaign_target", "campaign_cost", "campaign_control",
                  "campaign_result", "campaign_daily", "campaign_unmapped", "campaign_issue", "pre_calib"],
    ...
    dict(
        ...
        name="Zalo OA · Tổng người quan tâm (nhập tay)", cadence="cumulative", required=False,
        pattern="Zalo_OA_Follower*.xlsx", example="Zalo_OA_Follower_2026.xlsx",
        how="Export Tổng quan KHÔNG có tổng follower. Mỗi cuối tháng (hoặc cuối tuần) mở OA Manager › Thống kê › Người quan tâm, chép 'Tổng người quan tâm' (và 'Bỏ quan tâm' nếu có) vào sheet Follower: MỘT dòng MỘT ngày. Ô chưa có số để TRỐNG. Khi OpenAPI getoa chạy được, số tự động sẽ thay sổ này.",
        produces=["oa_follower"], via="tools/build_month.py", modules=["M8.1"],
        template="_MAU_Zalo_OA_Follower.xlsx",
    ),
    dict(
    ...
    dict(
        ...
        name="Danh mục đối tác — Partner + Aggregator", cadence="config", required=False,
        pattern="NOIRE_Doi_Tac*.xlsx", example="NOIRE_Doi_Tac_Partner_Aggregator.xlsx",
        how="Danh mục đối tác — MỘT file cho cả hai kênh (tạo bằng python tools/partner_template.py): 1_PARTNER · 2_AGGREGATOR (hợp đồng, phí, hoa hồng, kỳ hạn) · 3_CHUONG_TRINH (cơ chế, nội dung ưu đãi, kỳ chạy, Campaign ID) · 4_KE_HOACH. Số aggregator theo tháng ở 05_DOI_TAC/03_Aggregator.",
        produces=["partners", "partner_program", "partner_plan"], via="build_mkt.py", modules=["M7", "M9"],
    ),
    dict(
        id="S21_evoucher", group="05_DOI_TAC", dir="05_DOI_TAC/02_eVoucher_Doi_Tac",
        name="Log eVoucher đối tác (Techcombank × OneU…)", cadence="cumulative", required=False,
        pattern="eVoucher*.xlsx",
        example="eVoucher NCB - Techcombank Reward × OneU_T9.2026.xlsx",
        how="iPOS › Voucher › xuất log theo chiến dịch của mã đối tác. Mỗi chiến dịch một file, xuất từ ĐẦU chương trình tới hôm nay, thả đè bản cũ. Tháng phát / tháng dùng lấy từ NGÀY trong log, không lấy từ tên file. Gắn vào đối tác qua Campaign ID ở 3_CHUONG_TRINH.",
        produces=["partner_voucher"], via="build_mkt.py", modules=["M9"],
    ),
    dict(
        id="S19_aggregator", group="05_DOI_TAC", dir="05_DOI_TAC/03_Aggregator",
        name="Số Aggregator theo tháng (tự thống kê)", cadence="config", required=False,
        pattern="NOIRE_Aggregator*.xlsx", example="NOIRE_Aggregator_Theo_Thang.xlsx",
...
HIGHGATE_ORIGIN = {
    ...
    "S26_zalo_follower": "03 Customer Engagement/02 Loyalty Program/Camp Loyalty report/02. Data CRM/04. KPI Actual/01. OA Zalo",
...
SCHEMA = {
    ...
    "S26_zalo_follower": dict(sheets=[["Follower", None]], cols=["Ngày", "Tổng người quan tâm"]),
    ...
    "S15_partnership": dict(sheets=[["1_PARTNER"], ["2_AGGREGATOR"], ["3_CHUONG_TRINH"], ["4_KE_HOACH"]]),
    "S21_evoucher": dict(sheets=[None], cols=["Mã khuyến mãi", "Chương trình", "Trạng thái", "Ngày phát hành mã",
                                               "Ngày sử dụng", "Nhà hàng sử dụng", "Tiền giảm giá"]),
